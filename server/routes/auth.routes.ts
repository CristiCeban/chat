...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const {first_name, last_name, email, picture, id} = fetchedUser.data
            ...
            if (email) {
                ...
                if (!user) {
                    ...
                    const newUser = new User({email, first_name, last_name, password: hashedPassword, facebookId: id})
                    await newUser.save()
                    const newUserFound = await User.findOne({email}, {password: 0, "__v": 0})
                    const token = Utils.createToken(newUserFound.id)
                ...
                //create user
                if (!user) {
                    const hashedPassword = await bcrypt.hash(id, config.get('bcryptSalt'))
                    const newUser = new User({facebookId: id, first_name, last_name, password: hashedPassword})
                    await newUser.save()
